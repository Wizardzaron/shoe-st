...
try:
    ...
    for index, p in enumerate(rows):
        ...
        currentOrdersOfMonth = p["dateoforder"].strftime("%m")

        #print("Month: ", currentOrdersDate)

        #print(p["price"])
        #print(currentMonth)
        if currentOrdersOfMonth == currentMonth:
            #print("Hello")
            if p["customer_id"] == prevCustomer or prevCustomer is None:
                ...
                monthToDate = """UPDATE customer SET mtd = %s WHERE id = %s"""
                cur.execute(monthToDate, [priceSum, prevCustomer])
                conn.commit()
        ...
        if index == len(rows) - 1:
            print(priceSum)
            monthToDate = """UPDATE customer SET mtd = %s WHERE id = %s"""
            cur.execute(monthToDate, [priceSum, p["customer_id"]])
            conn.commit()
